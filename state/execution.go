...
func (s *State) ValExecBlock(eventsPub types.EventsPublisher, proxyAppConn proxy.AppConnConsensus, block *types.Block) (*ABCIResponses, error) {
	// Validate the block.
	...
	abciResponses, err := execBlockOnProxyApp(eventsPub, proxyAppConn, block, s.logger)
	if err != nil {
...
func execBlockOnProxyApp(eventsPub types.EventsPublisher, proxyAppConn proxy.AppConnConsensus, block *types.Block, logger log.Logger) (*ABCIResponses, error) {
	...
	proxyCb := func(req *abci.Request, res *abci.Response) {
		...
		case *abci.Response_DeliverTx:
			...
			types.FireEventTx(eventsPub, event)
		}
	}
...
func (s *State) ApplyBlock(eventsPub types.EventsPublisher, proxyAppConn proxy.AppConnConsensus,
	block *types.Block, partsHeader types.PartSetHeader, mempool types.Mempool) error {

	abciResponses, err := s.ValExecBlock(eventsPub, proxyAppConn, block)
	if err != nil {
...
func ExecCommitBlock(appConnConsensus proxy.AppConnConsensus, block *types.Block, logger log.Logger) ([]byte, error) {
	...
	_, err := execBlockOnProxyApp(eventsPub, appConnConsensus, block, logger)
	if err != nil {
